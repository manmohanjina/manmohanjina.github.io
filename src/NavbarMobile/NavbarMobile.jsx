...
import {ChevronDownIcon,HamburgerIcon,CloseIcon,SunIcon,MoonIcon}from "@chakra-ui/icons"
import { Link,Flex,RadioGroup,Stack ,Radio,Button,Drawer,DrawerBody,DrawerOverlay,DrawerContent, useToast,} from "@chakra-ui/react"
...
export default function NavbarMobile(){
  ...
  const hadnelPdf=()=>{
    ...
    timerRef.current=setTimeout(()=>{
      ...
      window.open("https://drive.google.com/file/d/1Ad_jxJd79WzjLj6B4ODzJCSO7ZVh43pV/view?usp=share_link")
...
console.log(state);
  return <Box style={{zIndex:1}} display={'flex'} bg="blackAlpha.600" justifyContent='space-between' p='2%' sx={{ position: '-webkit-sticky', /* Safari */ position: 'sticky', top: '0', }}  > 

<h1 style={{color:"white",padding:"10px"}}  >Manmohan</h1>
<button  style={{paddingLeft:'40%'}}  onClick={toggle} >{state?<MoonIcon boxSize={6} />:<SunIcon color={"yellow"} boxSize={6} />}</button>
...
<Box display={'flex'} width={'80%'}  flexDirection="column" gap={10} justifyContent='center' alignItems={'center'} mt='50px' >
  ...
  <a href="#resume" ><Button  varient='outline'  colorScheme='blackAlpha' onClose={onClose}   onClick={hadnelPdf} >Resume</Button></a>
